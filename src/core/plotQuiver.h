...
void PlotQuiver<T>::initialize(anpi::Matrix<T> data){
	...
	auxFun.append("	for x in range(len(Z)):\n");
	auxFun.append("		for y in range(len(Z[0])):\n");
	auxFun.append("			if(x!=0 and x!=len(Z)-1 and y!=0 and y!=len(Z[0])-1):\n");
	auxFun.append("				v[x][y]= k*(Z[x+1][y]-Z[x-1][y])/(2*delX)\n");
	auxFun.append("				u[x][y]= -k*(Z[x][y+1]-Z[x][y-1])/(2*delY)\n"	);	
	auxFun.append("			else:\n");
	auxFun.append("				v[x][y]= k/2*delX\n");
	auxFun.append("				u[x][y]= -k/2*delY\n");
	auxFun.append("			y+=5\n");
